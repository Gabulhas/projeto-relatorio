...
como as tecnologias utilizadas e a razão pela sua escolha.
É feita a descrição dos pontos de maior interesse da implementação do \emph{Node} do sistema, que foi desenvolvida após a escrita da Especificação (\ref{chap:especificacao}). 
...
\emph{Multiplexer} de pedidos \acs{HTTP}. Esta biblioteca da linguagem \emph{Go} foi utilizada para simplificar a declaração de caminhos do servidor \acs{HTTP}. É usada nos \emph{Nodes} e no servidor \acs{HTTP} da visualização.
...
É possível observar o uso deste sistema de construção no seguinte vídeo que se encontra disponibilizado na plataforma \emph{YouTube}. \url{https://youtu.be/D1UuCL_JIBI}
...
% Referir a outra secção
O acesso à secção crítica deste método é sincronizado com o uso do \emph{Mutex} ``Mutex'', isto é,
...
Por último, o método ``Idle'' da classe \emph{Node} irá ser executado, que mantém o tipo do \emph{Node}, mas
...
O acesso à secção crítica deste método é sincronizado com o uso do \emph{Mutex} ``Mutex'', ou seja,
...
o acesso a esta secção crítica é sincronizado com o uso de um \emph{Mutex}.
...
\subsubsection*{/find}
Este método equivale à saída do channel ``Find'' (referido no capítulo \ref{chap:especificacao}).
...
Este método equivale à saída do channel ``MyChan'' (como descrito no capítulo \ref{chap:especificacao}).
