...
como as tecnologias utilizadas, em que é feita uma descrição da razão de escolha de cada uma destas
a implementação do \emph{Node} do sistema
o foco deste capítulo é atribuído ao processo de desenvolvimento dos \emph{Nodes} do sistema, ou o programa que define um \emph{Node}, pois o conjunto destes descreve o protocolo em questão, e por último a implementação da visualização, mais especificamente o tratamento de informação que estará disponível neste componente. 
Este sistema é executado num contexto distribuído, isto é, várias instâncias do programa que define o \emph{Node} podem ser executadas em várias máquinas diferentes, sendo que é necessário a comunicação entre os \emph{Nodes} para o funcionamento do sistema.
...
As possíveis transformações são demonstradas na imagem 
...
No entanto, esta implementação tem limitações. 
\begin{itemize}
	\item 
...
Há duas soluções para o cálculo da fila, sendo que essas soluções têm os seus problemas, provocados pelos limitações da atualização dos estados do \emph{Nodes} referidos na introdução desta secção.

As duas soluções (cada uma com as suas limitações) são:
...
\section{Conclusões}
Neste capítulo foi apresentado todo o processo que envolveu a implementação deste projeto, desde as tecnologias, como a implementação do programa do \emph{Node} e da visualização.
...
Na implementação da interface, explicou-se a dificuldade na representação do estado da rede e da formação dos dados apresentados, devido, principalmente, ao facto das atualizações de cada \emph{Node} não chegam ao \emph{Node} de visualização por ordem de acontecimento. 
